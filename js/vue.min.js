...
let scale1 = devicewidth / 1920; // 分母——设计稿的尺寸
let scale2 = devicewidth / 1620; // 分母——设计稿的尺寸
if (devicewidth > 1280) {
...
let cookiebox = () =>{
  ...
  <div class='cookiezz' style="display: none;"></div>
  ...
  <div class='cookiebox' style="display: none;">
    ...
    <div class='cookiep'>
        请开启浏览器 cookies，或使用其他浏览器访问，否则测评不能正常进行!
    </div>
  ...
  function checkCookies() {
    ...
    document.cookie = "testcookie=test; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT";
  
    return cookiesEnabled;
  }

  if (!checkCookies()) {
    document.querySelector('.cookiezz').style.display = 'block'
    document.querySelector('.cookiebox').style.display = 'block'
  } 

  const isIOSWeChat = /MicroMessenger\/([\d\.]+)/i.test(navigator.userAgent) && /ip(hone|od|ad)/i.test(navigator.userAgent);
  if (isIOSWeChat) {
  let bot1 = document.querySelector('.togombti-bot') || null
  let bot2 = document.querySelector('.mphonebox ') || null
  let bot2img = document.querySelector('.mtjyjbox ') || null
  let botbox = document.createElement('div')
  botbox.style = 'position: fixed;bottom: 0;left: 0;width: 100%;height: 20px;background: #F8F8F8;z-index: 99'
  
    if (bot1) {
      bot1.style.bottom = '20px'
      document.querySelector('body').appendChild(botbox)
    }

    if (bot2) {
      bot1.style.bottom = '20px'
      bot2img.style.bottom = '70px'
      document.querySelector('body').appendChild(botbox)
    }
  }
 ...
 //检查cookies
//window.addEventListener('load', cookiebox);
...
function fontsize(){
  ...
  let scaleFactor = 16 / scaledFontSize;
